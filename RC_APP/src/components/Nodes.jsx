...
export default function PersonList() {
  ...
  const associateNode = () => {
    if (selectedNodeId && selectedNodeType) {
      navigate(`/associate/${selectedNodeId}/personne`);
    }
  };
                ...
                <TableRow hover role="checkbox" tabIndex={-1} key={row.id}>
                  ...
                  <TableCell align="left">
                    <Stack spacing={2} direction="row">
                      <AllInclusiveIcon style={{ fontSize: "20px", color: "orange", cursor: "pointer" }} onClick={() => handleClickOpen(row.id, 'person')} />
                      <InfoIcon style={{ fontSize: "20px", color: "green", cursor: "pointer" }} onClick={() => viewDetails(row.id, 'person')} />
                    </Stack>
                  </TableCell>
                </TableRow>
              ))}
